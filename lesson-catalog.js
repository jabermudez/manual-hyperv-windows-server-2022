(function renderLessonCatalog(){
  ...
  mount.innerHTML=phases.map((phase)=>{
    ...
    return `<section class="catalog-phase"><header><span>${String(lessons.indexOf(items[0])+1).padStart(2,'0')}–${String(lessons.indexOf(items.at(-1))+1).padStart(2,'0')}</span><h3>${phase}</h3></header><div class="lesson-card-grid">${items.map((lesson)=>`<a class="lesson-card ${saved[lesson.id]?'completed':''}" href="${window.lessonHref(lesson)}"><span class="lesson-card-number">${saved[lesson.id]?'✓':String(lesson.id).padStart(2,'0')}</span><div><small>Lección ${lesson.id}</small><strong>${lesson.title}</strong><p>${lesson.summary}</p><b>${saved[lesson.id]?'Completada · Revisar →':'Comenzar lección →'}</b></div></a>`).join('')}</div></section>`;
  }).join('');
